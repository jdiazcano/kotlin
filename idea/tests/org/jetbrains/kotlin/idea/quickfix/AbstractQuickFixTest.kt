...
abstract class AbstractQuickFixTest : KotlinLightCodeInsightFixtureTestCase() {
    ...
    protected fun doTest(beforeFileName: String) {
        ...
        doKotlinQuickFixTest(getTestName(false) + ".kt")
    ...
    private fun doKotlinQuickFixTest(testName: String) {
        val testFile = File(testDataPath, testName)
        CommandProcessor.getInstance().executeCommand(project, {
            ...
            try {
                ...
                applyAction(contents, testName, testFile.canonicalPath)
    ...
    private fun applyAction(contents: String, testName: String, testFullPath: String) {
        val fileName = testFullPath.substringAfterLast("/", "")
        val actionHint = ActionHint.parse(myFixture.file, contents.replace("\${file}", fileName))
